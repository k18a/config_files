...
you should place your code here."
  ...
  (spacemacs/set-leader-keys "ofc" 'calendar) ;open calendar

  (spacemacs/set-leader-keys "ofj" 'org-journal-new-entry) ;open journal file
  ...
  (defun open-lothian-paper()
    (interactive)
    (find-file "C:/Users/s1584407/papers/LothianShales/main.tex"))
  (spacemacs/set-leader-keys "ofl" 'open-lothian-paper) ;open bok file
  ...
  (require 'org-ref)
  (global-set-key (kbd "s-]") 'org-ref-insert-link) ;; insert org-ref link

	;; working with links and windows
  (defun save-and-open-link ()
    "save current buffer and open link in new window"
    (interactive)
    (save-buffer)
  ...
  (spacemacs/set-leader-keys "oo" 'save-and-open-link) ;save buffer and open link in new window
  (global-set-key (kbd "H-o") 'save-and-open-link)

  (defun save-and-open-link-same-window ()
    "save current buffer and open link in same window"
    (interactive)
    (save-buffer)
    (evil-goto-first-line)
    (bmkp-toggle-autonamed-bookmark-set/delete)
    ...
    (org-open-at-point)
    (delete-other-windows))
  (spacemacs/set-leader-keys "oO" 'save-and-open-link-same-window) ;save buffer and open link in same window
  (global-set-key (kbd "H-O") 'save-and-open-link-same-window)

  (defun save-and-previous-link-same-window ()
    "save current buffer and go to previous link in same window"
    (interactive)
    (save-buffer)
    (evil-goto-first-line)
    (bmkp-toggle-autonamed-bookmark-set/delete)
    (pop-global-mark)
    (org-mark-ring-goto))
  (spacemacs/set-leader-keys "oP" 'save-and-previous-link-same-window) ;save buffer and go to previous link in same window
  (global-set-key (kbd "H-P") 'save-and-previous-link-same-window)

  (defun save-and-previous-link ()
    "save current buffer and delete window"
    (interactive)
    (save-buffer)
    (evil-goto-first-line)
    (bmkp-toggle-autonamed-bookmark-set/delete)
  ...
  (spacemacs/set-leader-keys "op" 'save-and-previous-link) ;save buffer and delete window
  (global-set-key (kbd "H-p") 'save-and-previous-link)

  (spacemacs/set-leader-keys "oL" 'org-insert-link)
  (global-set-key (kbd "H-L") 'org-insert-link)

  ; org toggle latex fragment
  (global-set-key (kbd "H-l") 'org-toggle-latex-fragment)
  (spacemacs/set-leader-keys "ol" 'org-toggle-latex-fragment)
  ; start latex block
  (global-set-key (kbd "H-s") 'tex-latex-block)
  (spacemacs/set-leader-keys "os" 'tex-latex-block)
  ; end latex block
  (global-set-key (kbd "H-e") 'tex-latex-block)
  (spacemacs/set-leader-keys "oe" 'tex-latex-block)

  (setq org-startup-folded nil); turn of folding when document is opening
  ...
  ;; bookmarks settings
  (add-to-list 'load-path "~/bookmark-plus")
  (require 'bookmark+)

  (setq org-startup-folded nil); turn of folding when document is opening

  (org-defkey org-mode-map [(meta return)] 'org-meta-return); fix org links due to bug in recent emacs update
  ...
  (setq org-src-tab-acts-natively nil); let tab run as it should
  ...
  ;; pdf view
  (setq doc-view-ghostscript-program "c:/Program Files/gs/gs9.25/bin/gswin64c.exe")
...
?
#+END_EXPORT")
     ("IP" "#+BEGIN_SRC ipython :session :results raw drawer :ipyfile temp.png
%matplotlib inline
import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np
plt.style.use('dark_background')
sns.set_palette(sns.dark_palette(\"muted purple\",input=\"xkcd\",n_colors=10))
